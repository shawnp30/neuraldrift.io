...
export default function LabPage() {
  ...
  return <div className="nd-site"><div className="nd-shell nd-page">
    ...
    {TRACKED_MODELS.length > 0 && <section className="nd-section"><h2>Being investigated</h2><p className="nd-subtle">Newly released models NeuralDrift has researched but not yet run. These are documented, not execution-tested — see each page for exactly what has and hasn&apos;t been verified.</p><div className="nd-grid">{TRACKED_MODELS.map((model) => <article className="nd-card nd-card-body" key={model.slug}><p className="nd-eyebrow">{model.status.toUpperCase().replace(/_/g, " ")}</p><h3><Link href={`/lab/${model.slug}`}>{model.shortName}</Link></h3><p>{model.summary}</p><Link className="nd-text-link" href={`/lab/${model.slug}`}>Read the writeup →</Link></article>)}</div></section>}
